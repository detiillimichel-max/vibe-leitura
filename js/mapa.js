...
// MEMÓRIA DE COORDENADAS (Unificada p/ garantir a delineação)
...
// 3. SELECIONAR ROTA (DELINEAÇÃO + GAVETA)
function selecionarRota(id) {
    const rota = bancoDeRotas[id];
    const pontos = coordenadasDasRotas[id];

    // Desenha a linha delineada se houver pontos
    if (camadaDelineacao) { map.removeLayer(camadaDelineacao); }
    if (pontos) {
        camadaDelineacao = L.polyline(pontos, {
            color: rota.cor, weight: 12, opacity: 0.8 // Linha grossa para ver bem na rua
        }).addTo(map);
        map.fitBounds(camadaDelineacao.getBounds());
    }

    // SOBE A GAVETA DE LEITURA
...
function salvar() {
    ...
    alert("Vibe Leitura: Registro de " + (valor || "Hidrômetro não visível") + " salvo!");
